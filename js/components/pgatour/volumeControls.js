...
var VolumeControls = createReactClass({
  ...
  updateVolume: function (x) {
    var volume = ReactDOM.findDOMNode(this.volumeBar.current);
    var screenOffset = ReactDOM.findDOMNode(this).getBoundingClientRect() || { left: 0 };
    var position = x - screenOffset.left;
    var percentage = parseInt(100 * position / volume.clientWidth, 10);
    var volume_level = parseFloat(position / volume.clientWidth);
    if (percentage > 100) {
      volume_level = 1;
    }
    if (percentage < 10) {
  ...
  renderVolumeBars: function() {
    ...
    console.log('pgatour volume controls');
  ...
  render: function() {
    ...
    if (this.props.controller.state.audioOnly) {
      ...
      return this.renderPgatourVolumeBars();
    }
  }
});
